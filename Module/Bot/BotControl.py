...
class Answer:
    ...
    async def start_questions(self,
        ...
        asker = check.Asker(user_id=user_data.id,
                            asker_key=asker_key)
        asker_used = await asker.used_key()
        ...
        if not asker_used:
            ...
            send_message = await self.bot.send_message(chat_id=message.chat.id,
                                                       text=ask_data[0][1],
            ...
            send_message = await self.bot.send_message(chat_id=message.chat.id,
                                                       text=lang.asker_used)

            await asyncio.sleep(5)

            #Delete message
            await self.bot.delete_messages(chat_id=message.chat.id, message_ids=[message.message_id, send_message.message_id])
    ...
    async def confirmed_answer(self,
        ...
        if user_answers:
            ...
            await Action.confirm_user_answer(user_id=callback.from_user.id,
                                             ask_id=user_answer[0],
                                             asker_key=user_answer[1])
            ...
            await Action.add_user_answer_template(user_id=callback.from_user.id, 
                                                  ask_data=ask_data, 
                                                  asker_key=user_answer[1])
    ...
    async def no_confirmed_answer(self,
        ...
        if user_answers:
            ...
            await Action.reset_user_answer(user_id=callback.from_user.id,
                                           ask_id=user_answer[0],
                                           asker_key=user_answer[1])
